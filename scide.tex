...
The new IDE has a single-window design, Figure \ref{fig:mainwin} shows the default appearance on Ubuntu. It uses one
main code editing widget and docked widgets for the integrated help browser, the sclang post widget and the document
browser. These widgets can be docked to other positions of the main window or undocked and moved out of the main window,
e.g. to place them on a second screen or the like.

The code editor widget displays uses tabs to switch between multiple open documents and it can be split horizontally and
vertically to show more than one document. The status bar on the bottom of the main window is used to show the state of
the sclang interpreter and the default synthesis server. The server status window is a compact alternative to the
SuperCollider server window, showing status information like CPU utilization, number of synths, groups, synthdefs etc.
